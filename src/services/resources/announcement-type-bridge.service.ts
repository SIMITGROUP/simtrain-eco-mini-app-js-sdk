...
export class MiniAppAnnouncementTypeBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "announcementType";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.AnnouncementType[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.AnnouncementTypeAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
