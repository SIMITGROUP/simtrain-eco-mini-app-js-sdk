...
export class MiniAppProductBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "product";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.Product[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.Product>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.ProductAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
