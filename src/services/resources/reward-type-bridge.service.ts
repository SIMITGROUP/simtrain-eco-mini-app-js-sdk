...
export class MiniAppRewardTypeBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "rewardType";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.RewardType[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.RewardType>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.RewardTypeAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
