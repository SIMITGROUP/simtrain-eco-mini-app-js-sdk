...
export class MiniAppStopReasonBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "stopReason";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.StopReason[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.StopReason>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.StopReasonAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
