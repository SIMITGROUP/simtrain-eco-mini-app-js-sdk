...
export class MiniAppReligionBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "religion";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.Religion[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.Religion>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.ReligionAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
