...
export class MiniAppHolidayBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "holiday";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.Holiday[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.Holiday>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.HolidayAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
