...
export class MiniAppAgentBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "agent";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.Agent[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.Agent>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.AgentAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
