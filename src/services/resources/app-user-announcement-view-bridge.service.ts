...
export class MiniAppAppUserAnnouncementViewBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "appUserAnnouncementView";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.AppUserAnnouncementView[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.AppUserAnnouncementViewAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
