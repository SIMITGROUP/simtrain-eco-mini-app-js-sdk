...
export class MiniAppStudentGroupBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "studentGroup";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.StudentGroup[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.StudentGroupAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
