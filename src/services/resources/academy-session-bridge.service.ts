...
export class MiniAppAcademySessionBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "academySession";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.AcademySession[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.AcademySessionAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
