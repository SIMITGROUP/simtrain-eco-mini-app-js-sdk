...
export class MiniAppProductPackageBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "productPackage";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.ProductPackage[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.ProductPackageAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
