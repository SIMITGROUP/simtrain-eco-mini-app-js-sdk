...
export class MiniAppRaceBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "race";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.Race[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.Race>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.RaceAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
