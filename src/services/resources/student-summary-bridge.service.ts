...
export class MiniAppStudentSummaryBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "studentSummary";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.StudentSummary[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.StudentSummaryAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
