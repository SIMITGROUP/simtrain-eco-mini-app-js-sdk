...
export class MiniAppStudentSourceBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "studentSource";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.StudentSource[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.StudentSourceAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
