...
export class MiniAppAccountTransactionBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "accountTransaction";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.AccountTransaction[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.AccountTransactionAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
