...
export class MiniAppRoomTypeBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "roomType";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.RoomType[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.RoomType>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.RoomTypeAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
