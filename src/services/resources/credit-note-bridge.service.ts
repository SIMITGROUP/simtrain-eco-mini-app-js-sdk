...
export class MiniAppCreditNoteBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "creditNote";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.CreditNote[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.CreditNote>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.CreditNoteAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
