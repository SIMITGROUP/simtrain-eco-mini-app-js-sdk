...
export class MiniAppSchoolBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "school";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.School[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.School>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.SchoolAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
