...
export class MiniAppRoomBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "room";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.Room[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.Room>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.RoomAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
