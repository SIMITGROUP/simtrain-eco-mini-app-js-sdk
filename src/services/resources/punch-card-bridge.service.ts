...
export class MiniAppPunchCardBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "punchCard";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.PunchCard[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.PunchCard>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.PunchCardAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
