...
export class MiniAppTeacherGroupBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "teacherGroup";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.TeacherGroup[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.TeacherGroupAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
