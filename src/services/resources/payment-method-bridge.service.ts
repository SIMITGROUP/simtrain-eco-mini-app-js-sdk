...
export class MiniAppPaymentMethodBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "paymentMethod";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.PaymentMethod[]>(
      this.resourceName,
      "list",
      { body: params }
    );
  ...
  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.PaymentMethodAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
