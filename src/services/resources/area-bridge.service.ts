...
export class MiniAppAreaBridgeService {
  private bridge: MiniAppBridgeService;

  private resourceName = "area";
  ...
  async list(params?: MiniAppApiListParam | undefined) {
    return this.bridge.callApi<Schema.Area[]>(this.resourceName, "list", {
      body: params,
    });
  }

  async detail(id: string) {
    return this.bridge.callApi<Schema.Area>(this.resourceName, "detail", {
      id,
    });
  }

  async autoComplete(query: string, data: any) {
    return this.bridge.callApi<Schema.AreaAutoComplete[]>(
      this.resourceName,
      "autoComplete",
      {
        query,
