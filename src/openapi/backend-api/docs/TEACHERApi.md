...
|[**runCreateMany**](#runcreatemany) | **POST** /teacher/bulk-create | |
...
# **runCreateMany**
> Array<Teacher> runCreateMany(teacher)
...
let teacher: Array<Teacher>; //Data
...
const { status, data } = await apiInstance.runGetTeacherByLvl(
    dateFrom,
    dateTo
);
```
...
const { status, data } = await apiInstance.runGetTeacherCommissionResult(
    period
);
```
...
const { status, data } = await apiInstance.runGetTeacherCommissionSetting(
    id
);
```
