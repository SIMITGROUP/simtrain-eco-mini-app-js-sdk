...
```typescript
import { REWARDTYPEApi, Configuration } from "./api";
...
```typescript
import { REWARDTYPEApi, Configuration, RewardType } from "./api";
...
const { status, data } = await apiInstance.runCreate(rewardType);
```
...
const { status, data } = await apiInstance.runCreateMany(rewardType);
```
...
const { status, data } = await apiInstance.runPatch(id, rewardType);
```
...
```typescript
import { REWARDTYPEApi, Configuration, PatchManyRequest } from "./api";
...
const { status, data } = await apiInstance.runUpdate(id, rewardType);
```
