...
const { status, data } = await apiInstance.runCreateMany(student);
```
...
```typescript
import { STUDENTApi, Configuration, DeletePackageEnrollment } from "./api";
...
const { status, data } = await apiInstance.runSearchStudentAlphabetIndex(body);
```
...
const { status, data } = await apiInstance.runSearchStudentListPaginated(body);
```
