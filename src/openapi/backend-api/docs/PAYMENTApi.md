...
|[**runCreateMany**](#runcreatemany) | **POST** /payment/bulk-create | |
...
# **runCreateMany**
> Array<Payment> runCreateMany(payment)
...
let payment: Array<Payment>; //Data
...
const { status, data } = await apiInstance.runCreateOnlinePaymentReceipt(
    body
);
```
...
const { status, data } = await apiInstance.runGetDailyCollectionByPaymentType(
    date
);
```
...
const { status, data } = await apiInstance.runRecreateOnlinePaymentReceipt(
    body
);
```
...
const { status, data } = await apiInstance.runTotalPaymentAmount(
    month
);
```
